    ...
    stage('Build') {
        ...
        withEnv(["MVN_HOME=$mvnHome"]) {
            if (isUnix()) {
                ...
                sh '"$MVN_HOME/bin/mvn" -Dmaven.test.failure.ignore clean deploy sonar:sonar -Dsonar.host.url="http://http://ec2-13-233-61-131.ap-south-1.compute.amazonaws.com:9000"'
            } else {
